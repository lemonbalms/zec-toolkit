 ...
 *   1. .zec/deliverables.json (project-specific overrides)
...
function loadDeliverableConfig(directory) {
  ...
  const projectConfig = join(directory, '.zec', 'deliverables.json');
...
function detectStage(directory, sessionId) {
  ...
  if (sessionId) {
    const sessionState = join(directory, '.zec', 'state', 'sessions', sessionId, 'team-state.json');
  ...
  const legacyState = join(directory, '.zec', 'state', 'team-state.json');
