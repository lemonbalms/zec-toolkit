...
function activateState(directory, prompt, stateName, sessionId) {
  ...
  if (sessionId && /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$/.test(sessionId)) {
    const sessionDir = join(directory, '.zec', 'state', 'sessions', sessionId);
  ...
  // Fallback: write to legacy local .zec/state directory (no valid sessionId)
  const localDir = join(directory, '.zec', 'state');
...
function clearStateFiles(directory, modeNames, sessionId) {
  for (const name of modeNames) {
    const localPath = join(directory, '.zec', 'state', `${name}-state.json`);
    const globalPath = join(homedir(), '.zec', 'state', `${name}-state.json`);
    ...
    if (sessionId && /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$/.test(sessionId)) {
      const sessionPath = join(directory, '.zec', 'state', 'sessions', sessionId, `${name}-state.json`);
...
function linkRalphTeam(directory, sessionId) {
  const getStatePath = (modeName) => {
    if (sessionId && /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$/.test(sessionId)) {
      return join(directory, '.zec', 'state', 'sessions', sessionId, `${modeName}-state.json`);
    }
    return join(directory, '.zec', 'state', `${modeName}-state.json`);
  };
