...
function getBashHistoryConfig() {
  try {
    const configPath = join(cfgDir, '.zec-config.json');
...
function getAgentCompletionSummary(directory) {
  const trackingFile = join(directory, '.zec', 'state', 'subagent-tracking.json');
  try {
    if (existsSync(trackingFile)) {
      ...
      const parts = [];
      if (running.length > 0) {
        parts.push(`Running: ${running.length} [${running.map(a => a.agent_type.replace('zec:', '')).join(', ')}]`);
      }
