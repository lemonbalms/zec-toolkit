---
paths: "**/.zec/config/**,**/.mcp.json,**/.claude/settings.json,**/.claude/settings.local.json"
---

# Settings Management

Claude Code and ZEC configuration management rules.
...
Standard MCP servers in ZEC-ADK:
...
`.zec/config/` - ZEC-specific settings:
...
Team behavior is controlled by the `workflow.team` section in `.zec/config/sections/workflow.yaml`:
