...
```json
{
  "hooks": {
    "SessionStart": [{
      "type": "command",
      "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-session-start.sh\"",
      "timeout": 5
    }],
    "PreCompact": [{
      "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-compact.sh\"",
      "timeout": 5
    }],
    "PreToolUse": [{
      "matcher": "Write|Edit|Bash",
      "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-pre-tool.sh\"",
      "timeout": 5
    }],
    "PostToolUse": [{
      "matcher": "Write|Edit",
      "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-post-tool.sh\"",
      "timeout": 60
    }],
    "Stop": [{
      "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-stop.sh\"",
      "timeout": 5
    }],
    "TeammateIdle": [{
      "hooks": [{
        "type": "command",
        "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\"",
        "timeout": 10
      }]
    }],
    "TaskCompleted": [{
...
```json
{
  "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/hook.sh\""
}
```

**Important**: Quote the entire path to handle project folders with spaces:
- Correct: `"\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/hook.sh\""`
- Wrong: `"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/hook.sh"`

For StatusLine path configuration, see @settings-management.md (StatusLine does NOT support environment variables).

## Hook Wrappers

ZEC-ADK generates hook wrapper scripts during `zec init` that:

1. Read stdin JSON from Claude Code
2. Forward it to the zec binary via `zec hook <event>` command
3. Support multiple zec binary locations:
   - `zec` command in PATH
   - Detected Go bin path from initialization
   - Default `~/go/bin/zec`

Wrapper scripts are located at:
- `.claude/hooks/zec/handle-session-start.sh`
- `.claude/hooks/zec/handle-compact.sh`
- `.claude/hooks/zec/handle-pre-tool.sh`
- `.claude/hooks/zec/handle-post-tool.sh`
- `.claude/hooks/zec/handle-stop.sh`
- `.claude/hooks/zec/handle-agent-hook.sh`: TeammateIdle, TaskCompleted events (team mode)
...
2. Check if file has `.zec/config/sections/mx.yaml` exclusion
...
if [[ "$TOOL_NAME" =~ ^(Write|Edit)$ ]] && is_source_file "$FILE_PATH"; then
  ...
  zec mx check --file "$FILE_PATH" --dry
fi
```
...
- Use `/zec mx --dry` to preview tag recommendations
