...
This rule applies to all agents working with source code in the supported programming languages. For full @MX protocol details, see .claude/skills/zec/references/mx-tag.md.
...
Files matching patterns in `.zec/config/sections/mx.yaml` exclude list are not tagged:
...
Per-file limits from `.zec/config/sections/mx.yaml` (defaults):
- `anchor_per_file`: 3
- `warn_per_file`: 5
...
Project-level settings in `.zec/config/sections/mx.yaml`:
...
**IMPORTANT**: @MX tag descriptions MUST respect the `code_comments` setting from `.zec/config/sections/language.yaml`.
...
Before adding @MX tags, agents MUST read `.zec/config/sections/language.yaml` and use the `code_comments` value to determine the tag language.
