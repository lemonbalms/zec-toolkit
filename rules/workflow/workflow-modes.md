...
Development methodology reference for ZEC-ADK SPEC workflow.

For phase overview, token strategy, and transitions, see @spec-workflow.md

## Methodology Selection

The Run Phase adapts its workflow based on `quality.development_mode` in `.zec/config/sections/quality.yaml`:
...
- After IMPROVE: Skill("simplify") executes automatically (see run.md Phase 2.10). This is mandatory and not a separate step for the agent — it is orchestrated by ZEC.
...
- TRUST 5 gates passed (see .claude/rules/zec/core/zec-constitution.md)
...
- After REFACTOR: Skill("simplify") executes automatically (see run.md Phase 2.10). This is mandatory and not a separate step for the agent — it is orchestrated by ZEC.
...
### Auto-Detection (via /zec project or /zec init)
...
- During init: Use `zec init --mode <ddd|tdd>` flag (default: tdd)
- After project setup: Re-run `/zec project` to auto-detect based on codebase analysis
- Manual edit: Edit `quality.development_mode` in `.zec/config/sections/quality.yaml`
