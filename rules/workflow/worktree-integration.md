---
paths: "**/.claude/agents/**,**/.zec/worktrees/**"
---

# Worktree Integration Guide

Integration guide for ZEC Worktree and Claude Code Native Worktree systems.

## Overview

ZEC-ADK supports two complementary worktree systems for isolated development:
...
**ZEC Worktree** (`~/.zec/worktrees/{ProjectName}/`):
- Persistent, SPEC-scoped workspaces in global home directory
- Managed via `zec worktree` CLI commands
- Used for multi-session SPEC development and team collaboration

## Comparison Table

| Feature | Claude Native | ZEC |
|---------|--------------|------|
| **Path** | `.claude/worktrees/<name>/` | `~/.zec/worktrees/{Project}/{SPEC}/` |
...
| **CLI** | `claude -w` (user) or `isolation: worktree` (agent) | `zec worktree new/list/remove` |
| **Cleanup** | Automatic on session end | Manual via `zec worktree remove` |
...
Is this a multi-session SPEC development?
  YES → Use ZEC Worktree (zec worktree new SPEC-XXX)
  NO ↓
...
- [HARD] GitHub workflow agents (fixer agents in /zec github issues) MUST use `isolation: "worktree"` for branch isolation
...
### Use ZEC Worktree (`zec worktree`) for:
...
ZEC-ADK implements hook handlers for worktree lifecycle events:

| Hook Event | Triggered When | ZEC Handler |
|-----------|---------------|--------------|
| WorktreeCreate | Agent with isolation: worktree spawns | `zec hook worktree-create` |
| WorktreeRemove | Agent with isolation: worktree terminates | `zec hook worktree-remove` |

Hook scripts are located at:
- `.claude/hooks/zec/handle-worktree-create.sh`
- `.claude/hooks/zec/handle-worktree-remove.sh`
...
|-------|-------|----------|
| Worktree not found | Removed manually | Run `zec worktree list` to verify |
...
| Hooks not firing | Missing wrapper script | Check `.claude/hooks/zec/` directory |
...
| Run | ZEC | `~/.zec/worktrees/{Project}/{SPEC}/` |
| Sync | ZEC | Same as Run phase |
