---
paths: "**/.zec/specs/**,**/.zec/config/sections/quality.yaml"
---

# SPEC Workflow

ZEC's three-phase development workflow with token budget management.

## Phase Overview

| Phase | Command | Agent | Token Budget | Purpose |
|-------|---------|-------|--------------|---------|
| Plan | /zec plan | manager-spec | 30K | Create SPEC document |
| Run | /zec run | manager-ddd/tdd (per quality.yaml) | 180K | DDD/TDD implementation |
| Sync | /zec sync | manager-docs | 40K | Documentation sync |
...
Output:
- Research document at `.zec/specs/SPEC-XXX/research.md` (deep codebase analysis)
- SPEC document at `.zec/specs/SPEC-XXX/spec.md`
...
- Agent returns structured stagnation report to ZEC (agents cannot call AskUserQuestion)
- ZEC presents gap analysis to user via AskUserQuestion with options:
...
Detection method:
- Append acceptance criteria completion count and error count delta to `.zec/specs/SPEC-{ID}/progress.md` at the end of each iteration
...
- `<zec>DONE</zec>` - Task complete
- `<zec>COMPLETE</zec>` - Full completion

## Context Management

/clear Strategy:
- After /zec plan completion (mandatory)
...
- Action: Execute /clear, then /zec run SPEC-XXX

Run to Sync:
- Trigger: Implementation complete, tests passing
- Action: Execute /zec sync SPEC-XXX
...
- ZEC runs annotation cycle with user for plan refinement (1-6 iterations)
- ZEC synthesizes into SPEC document
...
- Plan phase: .claude/skills/zec/team/plan.md
- Run phase: .claude/skills/zec/team/run.md
- Fix phase: .claude/skills/zec/team/debug.md
- Review: .claude/skills/zec/team/review.md
...
- `workflow.team.enabled: true` in `.zec/config/sections/workflow.yaml`
