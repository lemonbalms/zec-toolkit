...
Model policy is set via `zec init --model-policy <tier>`:
...
- Activation: `zec cg` (requires tmux)
...
ZEC's --deepthink flag triggers high effort for the current turn. This aligns with the "deepthink" keyword behavior in Claude Code.
