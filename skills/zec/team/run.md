...
Before executing this workflow, check `.zec/config/sections/llm.yaml`:
...
the tmux session has GLM env vars injected by `zec cg`.
...
1. **Delegate to manager-spec subagent**:
   ...
   Agent(
     ...
     prompt: "Create SPEC document for: {user_description}
              Follow EARS format.
              Output to: .zec/specs/SPEC-XXX/spec.md"
   )
   ```
...
3. **Output**: `.zec/specs/SPEC-XXX/spec.md`
...
ZEC monitors teammate progress:
