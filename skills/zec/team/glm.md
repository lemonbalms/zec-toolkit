---
name: zec-workflow-team
description: >
  CG (Claude + GLM) hybrid mode for ZEC-ADK. Uses tmux pane-level environment
...
# ZEC CG Mode (Claude + GLM via tmux Agent Teams)
...
Read `.zec/config/sections/llm.yaml` for `team_mode` value:
...
Detection steps:
1. Read `.zec/config/sections/llm.yaml`
...
5. **Run workflow**:
   ```
   /zec --team "Your task description"
   ```

## tmux Environment Variables

`zec cg` injects these into the tmux session:
...
|-------|------|-----|-------|
| Leader (ZEC) | Original | Claude | User's choice (Opus/Sonnet) |
