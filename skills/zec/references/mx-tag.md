...
For tag types, trigger conditions, lifecycle rules, when to add/update/remove tags, per-file limits, mandatory fields, and language settings, see the authoritative source: .claude/rules/zec/workflow/mx-tag-protocol.md
...
**WHEN** a file matches a pattern in `.zec/config/sections/mx.yaml` exclude list: The agent does not add, modify, or validate @MX tags in that file.
