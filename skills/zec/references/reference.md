...
description: >
  ...
  and resume patterns used across all ZEC workflows. Provides context
...
Common patterns, flag reference, and legacy command mapping used across all ZEC workflows.
...
- ZEC Workflow: Parallel exploration phase, then sequential SPEC generation and DDD/TDD implementation
...
- Read existing SPEC document from .zec/specs/SPEC-XXX/
...
Previous /zec:X-Y command format mapped to new /zec subcommand format:

- /zec:0-project maps to /zec project
- /zec:1-plan maps to /zec plan
- /zec:2-run maps to /zec run
- /zec:3-sync maps to /zec sync
...
- /zec:coverage maps to /zec coverage
- /zec:e2e maps to /zec e2e
- /zec:mx maps to /zec mx

Note: /zec:99-release is a separate local-only command, not part of the /zec skill.
