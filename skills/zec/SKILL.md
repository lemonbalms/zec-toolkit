...
- **codemaps**: Generate architecture documentation in `.zec/project/codemaps/`
...
### (default) - ZEC Autonomous Workflow
...
Output: product.md, structure.md, tech.md in .zec/project/
...
Before executing plan, run, sync, fix, loop, or default workflows, verify project documentation exists by checking for `.zec/project/product.md`. If product.md does NOT exist, use AskUserQuestion to ask the user (in their conversation_language):

Question: Project documentation not found. Would you like to create it first?
Options:
- Create project documentation (Recommended): Generates product.md, structure.md, tech.md through a guided interview. This helps ZEC understand your project context for better results in all subsequent workflows.
- Skip and continue: Proceed without project documentation. ZEC will have less context about your project.

This check does NOT apply to: project, feedback subcommands.

[HARD] Beginner-Friendly Option Design:
All AskUserQuestion calls throughout ZEC workflows MUST follow these rules:
...
Load relevant configuration from .zec/config/config.yaml and section files as needed.
...
When all workflow phases complete successfully, add the appropriate completion marker (`<zec>DONE</zec>` or `<zec>COMPLETE</zec>`).
