...
triggers:
  keywords: ["zec", "autonomous", "pipeline", "build", "implement", "create"]
  agents: ["zec"]
  phases: ["plan", "run", "sync"]
---

# Workflow: ZEC - Autonomous Development Orchestration

Purpose: Full autonomous workflow. User provides a goal, ZEC autonomously executes plan -> run -> sync pipeline. This is the default workflow when no subcommand is specified.
...
[HARD] Before Phase 2 implementation, ALWAYS check `.zec/config/sections/quality.yaml`:
...
- If .zec/project/codemaps/ exists: Use as architecture baseline to accelerate exploration (skip redundant scanning)
...
- Save research.md to .zec/specs/SPEC-{ID}/research.md when SPEC ID is determined
...
- Output: EARS-format SPEC document at .zec/specs/SPEC-XXX/spec.md
...
3. ZEC delegates to manager-spec: "Address all inline notes. DO NOT implement any code."
...
[HARD] Methodology selection based on `.zec/config/sections/quality.yaml`:
...
- Conditionally update project documents (.zec/project/) when structural changes detected
...
11.5. **Execution Mode Selection Gate**: After Phase 1.5 approval, before Phase 2
   - Read .zec/config/sections/llm.yaml → team_mode ("" = cc, "glm" = glm, "cg" = cg)
