...
For phase overview and token budgets, see: .claude/rules/zec/workflow/spec-workflow.md

## Scope

- Implements Steps 1-2 of ZEC's 4-step workflow (Intent Understanding, Plan Creation)
- Steps 3-4 are handled by /zec run and /zec sync respectively
...
- .zec/project/codemaps/ directory listing (architecture maps for existing codebase understanding)
- .zec/specs/ directory listing (existing SPECs for deduplication)

Pre-execution commands: git status, git branch, git log, git diff, find .zec/specs.
...
- If .zec/project/codemaps/ exists: Use as architecture baseline to accelerate exploration
- Find relevant files by keywords from user request
- Locate existing SPEC documents in .zec/specs/
...
Purpose: Produce a persistent research.md artifact documenting deep codebase understanding. This document serves as a verification surface — ZEC and the user can review it and correct misunderstandings before planning begins.
...
Output: `.zec/specs/SPEC-{ID}/research.md` containing:
...
- Check for duplicate SPECs in .zec/specs/
...
4. ZEC delegates to manager-spec subagent: "Address all inline notes in the plan document and update it accordingly. DO NOT implement any code."
5. manager-spec updates plan.md, removing addressed notes and incorporating feedback
6. ZEC presents updated plan to user for another review cycle
...
- Reports route to .zec/reports/, Documentation to .zec/docs/
...
- ID Uniqueness: Search .zec/specs/ to confirm no duplicates exist
...
*This issue was automatically created by ZEC from SPEC-{ID}.*
*SPEC location: `.zec/specs/SPEC-{ID}/spec.md`*
EOF
)" \
  --label "spec"
```
...
- Update `issue_number` field in `.zec/specs/SPEC-{ID}/spec.md` YAML frontmatter
- Add cross-reference comment in the Issue: `gh issue comment {number} --body "SPEC document: .zec/specs/SPEC-{ID}/spec.md"`
...
- Stage SPEC files: git add .zec/specs/SPEC-{ID}/
...
Options:
- Start Implementation (execute /zec run SPEC-{ID})
- Modify Plan
...
Read `.zec/config/sections/llm.yaml` → `llm.team_mode` field:
...
- 새 tmux 세션에서 worktree 디렉토리로 이동 후 `/zec run SPEC-{ID}` 실행.
...
- Directory naming follows .zec/specs/SPEC-{ID}/ format
