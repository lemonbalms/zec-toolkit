...
After Phase 2, before generating tests, ZEC MUST evaluate whether to use Skill("batch").

Condition: total_gap_count (P1 + P2 gaps) >= 10

Decision:

- If condition is met: Execute Skill("batch") directly. Batch mode assigns each gap file to an independent agent running in a git worktree. Each agent generates tests for its assigned file, runs them to verify, and reports results. ZEC collects all generated test files and proceeds to Phase 4.
...
- Fix remaining gaps (Recommended): Continue generating tests for uncovered areas until the target is met. ZEC will prioritize the highest-risk gaps.
...
- Phase 5: ZEC orchestrator (report and user interaction)
