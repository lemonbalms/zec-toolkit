...
- Implements Step 4 of ZEC's 4-step workflow (Report and Commit)
- Receives implementation artifacts from /zec run
...
- When `.zec/project/` documents are outdated
...
3. **Project Document Updates**:
   - Updates `.zec/project/tech.md` when new dependencies/technologies added
   - Updates `.zec/project/structure.md` when architecture changes
   - Updates `.zec/project/product.md` when new features added
   - Updates `.zec/project/codemaps/` when architecture changes detected (delegates to codemaps workflow)
...
- .zec/config/config.yaml (git strategy, language settings)
...
- .zec/specs/ directory listing (SPEC documents for sync)
- .zec/project/ directory listing (project documents for conditional update)
- .zec/project/codemaps/ directory listing (architecture maps for conditional update)
- README.md (current project documentation)

Pre-execution commands: git status, git diff, git branch, git log, find .zec/specs.
...
Purpose: Ensure code has appropriate @MX annotations for AI agent context. Supports all 16 ZEC-ADK languages.
...
2. Show message: "Run /zec run to add missing tags, or use --skip-mx to bypass"
...
- Create backup directory: .zec/backups/sync-{timestamp}/
- Copy critical files: README.md, docs/, .zec/specs/
...
- Generate sync report: .zec/reports/sync-report-{timestamp}.md
...
Purpose: Update .zec/project/ documents when significant structural changes are detected.
...
Skip condition: If .zec/project/ directory does not exist or contains no files, skip this step entirely.
...
- If architectural changes: Regenerate .zec/project/codemaps/ via codemaps workflow (workflows/codemaps.md) when significant structural changes (new directories, dependency graph changes, or module reorganization) are detected
...
Read `github.git_workflow` from `.zec/config/sections/system.yaml`. This determines how changes are delivered.
...
1. **Session Resumption**: When resuming development, `/zec context` retrieves this information automatically
...
- Context metadata saved to `.zec/state/sync-context-{SPEC-ID}.json` for quick access
...
- Review PR on GitHub
- Auto-Merge PR (/zec sync --merge)
- Create Next SPEC (/zec plan)
...
- Display retry command: /zec sync [mode]
- Exit with code 0
