...
For methodology details (DDD ANALYZE-PRESERVE-IMPROVE and TDD RED-GREEN-REFACTOR cycles, success criteria, brownfield enhancement), see: .claude/rules/zec/workflow/workflow-modes.md

## Scope

- Implements Step 3 of ZEC's 4-step workflow (Task Execution)
- Receives SPEC documents created by /zec plan
- Hands off to /zec sync for documentation and PR

## Input

- $ARGUMENTS: SPEC-ID to implement (e.g., SPEC-AUTH-001)
- Resume: Re-running /zec run SPEC-XXX resumes from last successful phase checkpoint
...
- .zec/config/config.yaml (git strategy, automation settings)
- .zec/config/sections/quality.yaml (coverage targets, TRUST 5 settings, development_mode)
- .zec/config/sections/git-strategy.yaml (auto_branch, branch creation policy)
- .zec/config/sections/language.yaml (git_commit_messages setting)
- .zec/specs/SPEC-{ID}/ directory (spec.md, plan.md, acceptance.md)
- .zec/specs/SPEC-{ID}/progress.md (session resume context: if exists, load to identify completed phases and skip them; if absent, will be created at Phase 1 start)
- .zec/project/structure.md (architecture context for implementation decisions)
- .zec/project/tech.md (technology stack context)
- .zec/project/codemaps/ directory listing (architecture maps for dependency and module understanding)

Pre-execution commands: git status, git branch, git log, git diff.

### Resume Check

Before Phase 1, check if `.zec/specs/SPEC-{ID}/progress.md` exists:
...
Input: SPEC document content from the provided SPEC-ID. If research.md exists in the SPEC directory (.zec/specs/SPEC-{ID}/research.md), include it as additional context for deeper understanding of the codebase architecture, reference implementations, and identified risks.
...
Progress update: Append to `.zec/specs/SPEC-{ID}/progress.md`:
```
...
See .claude/rules/zec/workflow/mx-tag-protocol.md for tag type definitions.

### Batch Mode Decision [MANDATORY EVALUATION]

Before routing to Phase 2, ZEC MUST evaluate whether to use Skill("batch") for parallel implementation.
...
- If ANY condition is met: Execute Skill("batch") directly. Batch mode replaces sequential Phase 2. Each batch unit executes one atomic task in an isolated git worktree, runs tests, and creates a PR. ZEC collects all PRs and merges in dependency order via manager-git.
...
Before Phase 2, determine the development methodology by reading `.zec/config/sections/quality.yaml`:
...
Input: Approved execution plan from Phase 1 plus task decomposition from Phase 1.5. Include `.zec/project/structure.md` and `.zec/project/tech.md` as onboarding context in the agent prompt so the implementation agent understands the project's architecture conventions before writing code.
...
This divergence data is consumed by /zec sync for SPEC document updates and project document synchronization.
...
Purpose: Detect stagnation and trigger re-assessment if implementation is stuck. See .claude/rules/zec/workflow/spec-workflow.md for trigger conditions, communication path, and detection method.

Check `.zec/specs/SPEC-{ID}/progress.md` for stagnation signals. If triggered, return structured stagnation report to ZEC for user escalation.
...
Purpose: Update @MX code annotations for modified files. See .claude/rules/zec/workflow/mx-tag-protocol.md for tag rules.
...
Action: ZEC MUST call Skill("simplify") at this phase. Do not delegate to a subagent. Call it directly.
...
Options:
- Sync Documentation (recommended): Execute /zec sync to synchronize docs and create PR
- Implement Another Feature: Return to /zec plan for additional SPEC
...
When the run phase is invoked from plan.md Decision Point 3.5 or zec.md Phase 11.5, the gate passes these parameters:
...
**If no execution_mode provided (direct `/zec run` invocation):**
...
See .claude/rules/zec/workflow/worktree-integration.md for the complete worktree decision tree.
...
- Phase 2 to /zec sync: Implementation divergence report enables accurate SPEC and project document updates
