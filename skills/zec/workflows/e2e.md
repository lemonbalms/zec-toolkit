...
For Playwright best practices and patterns, reference: Skill("zec-workflow-testing") which includes:
...
Journey Discovery:
- Read project documentation (.zec/project/product.md) for feature descriptions
...
- Define custom journey: Describe a custom user journey to test. ZEC will create the test script based on your description.
...
- Fix failing tests (Recommended): Debug and fix the failing E2E tests. ZEC will analyze the failure screenshots and error messages to identify root causes.
- Rerun failed tests: Retry only the failed tests with increased timeout. Useful for flaky tests that may pass on retry.
- Add more journeys: Define additional user journeys to improve E2E coverage. ZEC will suggest uncovered critical paths.
