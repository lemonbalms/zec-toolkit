...
- Review each item: Review each dead code candidate individually before deciding. ZEC will present them one by one for your approval.
...
After Phase 3 user approval, ZEC MUST evaluate whether to use Skill("batch") for removal.
...
- Phase 3: ZEC orchestrator (user approval via AskUserQuestion)
- Phase 4: expert-refactoring subagent (safe removal)
- Phase 5: expert-testing subagent (test verification)
- Phase 5.5: ZEC orchestrator or expert-refactoring (MX tag cleanup)
