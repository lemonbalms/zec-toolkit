...
- Marker types: `<zec>DONE</zec>`, `<zec>COMPLETE</zec>`
...
- If session duration exceeds 25 minutes OR iteration time doubling:
  - Save proactive checkpoint to $CLAUDE_PROJECT_DIR/.zec/cache/loop-snapshots/memory-pressure.json
  - Warn user about memory pressure
  - Suggest resuming with /zec:loop --resume memory-pressure
...
- See .claude/rules/zec/workflow/mx-tag-protocol.md for tag rules

Step 8 - Snapshot Save:
- Save iteration snapshot to $CLAUDE_PROJECT_DIR/.zec/cache/loop-snapshots/
...
Snapshot location: $CLAUDE_PROJECT_DIR/.zec/cache/loop-snapshots/
...
Loop state file: $CLAUDE_PROJECT_DIR/.zec/cache/.zec_loop_state.json

Resume commands:
- /zec:loop --resume latest
- /zec:loop --resume iteration-002
- /zec:loop --resume memory-pressure
