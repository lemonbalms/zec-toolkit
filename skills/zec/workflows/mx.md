...
description: >
  ...
  Supports all 16 ZEC-ADK languages with language-aware comment syntax.
...
For tag types, lifecycle rules, mandatory fields, and per-file limits, see: .claude/rules/zec/workflow/mx-tag-protocol.md
...
- During `/zec sync` for MX validation
...
2. **Project Context Loading**
   - Read `.zec/project/tech.md` for tech stack context
   - Read `.zec/project/structure.md` for architecture context
   - Read `.zec/project/product.md` for feature context
...
- Project context: Loaded from .zec/project/tech.md
...
/zec mx --all --lang go,python

# Lower threshold for more coverage
/zec mx --all --threshold 2
```
