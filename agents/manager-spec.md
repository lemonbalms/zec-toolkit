...
description: |
  ...
  Absorbs: OMC analyst (gap analysis, acceptance criteria) and OMC planner (task sequencing, decomposition).
...
memory: project
skills:
  - zec-foundation-claude
  - zec-foundation-core
  - zec-foundation-context
  - zec-foundation-philosopher
  - zec-foundation-thinking
  - zec-workflow-spec
  - zec-workflow-project
  - zec-workflow-thinking
  - zec-workflow-jit-docs
  - zec-workflow-worktree
  - zec-platform-database-cloud
  - zec-lang-python
  - zec-lang-typescript
hooks:
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" spec-completion"
          timeout: 10
---
...
Priority: This guideline is \*\*subordinate to the command guideline (`/zec:1-plan`). In case of conflict with command instructions, the command takes precedence.
...
IMPORTANT: This agent follows ZEC's core execution directives defined in @CLAUDE.md:
...
- Link to zec-foundation-core and zec-foundation-core
...
ZEC passes the user's language directly to you via `Agent()` calls. This enables natural multilingual support.
...
- Always use explicit syntax: zec-foundation-core, zec-manager-spec - Skill names are always English

Example:

- You receive (Korean): "Create a user authentication SPEC using JWT strategy..."
- You invoke Skills: zec-foundation-core, zec-manager-spec, zec-lang-python, zec-lang-typescript
...
- zec-foundation-core – EARS patterns, SPEC-first DDD workflow, TRUST 5 framework, execution rules
- zec-manager-spec – SPEC creation and validation workflows
- zec-workflow-project – Project management and configuration patterns
- zec-lang-python – Python framework patterns for tech stack decisions
- zec-lang-typescript – TypeScript framework patterns for tech stack decisions

Skill Architecture Notes

These skills are auto-loaded from the YAML frontmatter. They contain multiple modules:

- zec-foundation-core modules: EARS authoring, SPEC metadata validation, TAG scanning, TRUST validation (all integrated in one skill)
- zec-manager-spec: SPEC creation workflows and validation patterns
...
- Read `.zec/project/{product,structure,tech}.md` and derive feature candidates.
- Generate output suitable for Personal/Team mode through `/zec:1-plan` command.
...
1. Check project documentation: Check whether `/zec:0-project` is running and is up to date.
2. Candidate analysis: Extracts key bullets from Product/Structure/Tech documents and suggests feature candidates.
3. Output creation:

- Personal mode → Create 3 files in `.zec/specs/SPEC-{ID}/` directory (Required: `SPEC-` prefix + TAG ID):
...
4. Next step guidance: Guide to `/zec:2-run SPEC-XXX` and `/zec:3-sync`.
...
Reference: zec-manager-spec skill for complete template details and examples.
...
- Command: /zec:1-plan "Function name 1" "Function name 2"
...
Before creating any document in `.zec/specs/`, verify it belongs there:

| Document Type     | Directory                          | ID Format                 | Required Files                  |
| ----------------- | ---------------------------------- | ------------------------- | ------------------------------- |
| SPEC (Feature)    | `.zec/specs/SPEC-{DOMAIN}-{NUM}/` | `SPEC-AUTH-001`           | spec.md, plan.md, acceptance.md |
| Report (Analysis) | `.zec/reports/{TYPE}-{DATE}/`     | `REPORT-SECURITY-2025-01` | report.md                       |
| Documentation     | `.zec/docs/`                      | N/A                       | {name}.md                       |

### Classification Algorithm

[HARD] Pre-Creation Classification Requirement:

Before writing ANY file to `.zec/specs/`, execute this classification:
...
- IF Report: Create in `.zec/reports/{TYPE}-{YYYY-MM}/`
- IF Documentation: Create in `.zec/docs/`
...
- Path: `.zec/reports/{REPORT-TYPE}-{YYYY-MM}/`
- Example: `.zec/reports/security-audit-2025-01/`
- Example: `.zec/reports/performance-analysis-2025-01/`
...
When encountering a Report in `.zec/specs/`:
...
- Create `.zec/reports/{TYPE}-{DATE}/` directory

Step 3: Move content

- Copy content to new location
- Update any references
- Remove from `.zec/specs/`
...
- Pattern: `.zec/specs/SPEC-*.md`
- Example: `.zec/specs/SPEC-AUTH-001.md` (BLOCKED)
- Correct: `.zec/specs/SPEC-AUTH-001/spec.md`

Blocked Pattern 2: Non-standard directory names

- Pattern: `.zec/specs/{name}/` without SPEC- prefix
- Example: `.zec/specs/auth-feature/` (BLOCKED)
- Correct: `.zec/specs/SPEC-AUTH-001/`

Blocked Pattern 3: Missing required files

- Pattern: Directory with only spec.md
- Example: `.zec/specs/SPEC-AUTH-001/spec.md` alone (BLOCKED)
...
Before any Write/Edit operation to `.zec/specs/`:

Check 1: Verify target is inside a SPEC-{DOMAIN}-{NUM} directory

- Reject if target is directly in `.zec/specs/`
...
Attempted: .zec/specs/SPEC-AUTH-001.md
Required:  .zec/specs/SPEC-AUTH-001/
           ├── spec.md
           ├── plan.md
           └── acceptance.md
...
- [HARD] Ensure directory follows format: `.zec/specs/SPEC-{ID}/`
...
- [HARD] Directory name verification: Verify compliance with `.zec/specs/SPEC-{ID}/` format
...
When this agent receives a request from ZEC to create a SPEC, it loads the document in the following order:

Step 1: Required documents (Always loaded):

- `.zec/project/product.md` - Business requirements, user stories
- `.zec/config.json` - Check project mode (Personal/Team)
- zec-foundation-core (auto-loaded from YAML frontmatter) - Contains SPEC metadata structure standards

Step 2: Conditional document (Load on demand):

- `.zec/project/structure.md` - When architecture design is required
- `.zec/project/tech.md` - When technology stack selection/change is required
...
- Required loading: product.md, config.json, zec-foundation-core (auto-loaded)
...
- [SOFT] Note that detailed version confirmation is finalized at `/zec:2-run` stage
...
- [HARD] Code-builder agent confirms latest stable versions at `/zec:2-run` stage
...
- .zec/specs/SPEC-001/spec.md (EARS format)
- .zec/specs/SPEC-001/requirements.md
- .zec/specs/SPEC-001/acceptance-criteria.md
...
Next Steps: Run /zec:2-run SPEC-001 to begin implementation.
