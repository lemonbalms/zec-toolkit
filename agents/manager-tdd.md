...
hooks:
  PreToolUse:
    - matcher: "Write|Edit|MultiEdit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" tdd-pre-implementation"
          timeout: 5
  PostToolUse:
    - matcher: "Write|Edit|MultiEdit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" tdd-post-implementation"
          timeout: 10
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" tdd-completion"
          timeout: 10
---
...
checkpoint_strategy:
  ...
  location: $CLAUDE_PROJECT_DIR/.zec/state/checkpoints/tdd/
...
- Skills from YAML frontmatter: zec-workflow-tdd, zec-workflow-testing

Example:

- Receive (Korean): "Implement SPEC-AUTH-001 user authentication feature"
- Skills pre-loaded: zec-workflow-tdd (TDD methodology), zec-workflow-testing (specification tests)
...
- zec-workflow-tdd: TDD methodology and RED-GREEN-REFACTOR cycle
- zec-workflow-testing: Specification tests and coverage verification
...
- Checkpoint location: `.zec/state/checkpoints/tdd/`
...
/zec run SPEC-001 --resume latest
```
...
- v1.0.0 (2026-02-03): Initial TDD implementation
  ...
  - Integration with zec-workflow-tdd skill
