...
Key Advantage: Simple, consistent GitHub Flow for all modes. Users select mode manually via `.zec/config.json` without auto-switching.
...
- Commit messages: Read git_commit_messages from .zec/config/sections/language.yaml
...
- Tag names: Always English (v1.0.0, zec_cp/20251203_120000) for version consistency
...
- zec-foundation-claude – Provides Claude Code agent patterns, hook integration, settings management
- zec-workflow-project – Provides Git workflow strategies, GitHub Flow patterns, project configuration
- zec-foundation-quality – Provides Git command patterns, validation scripts, error handling
...
2. Load zec-workflow-project skill for branch strategy
...
This is a dedicated agent that optimizes and processes all Git operations in ZEC-ADK for each mode.

## Configuration Loading [HARD]

Always load these configuration files at the start of every operation:

@.zec/config/sections/git-strategy.yaml
@.zec/config/sections/language.yaml
...
- Execute: `git tag -a "zec_cp/$(TZ=Asia/Seoul date +%Y%m%d_%H%M%S)" -m "Message"`
...
- Ensure all Git tasks follow TRUST principles from zec-core-dev-guide
...
- Manually enable via `.zec/config/config.yaml` configuration
...
File Location: `.zec/config/config.yaml`
...
1. When writing a SPEC (`/zec:1-plan`):
...
2. When implementing DDD (`/zec:2-run`):
...
3. When synchronization completes (`/zec:3-sync`):
...
- Execute: `git tag -a "zec_cp/[timestamp]" -m "[descriptive message]"`
...
- Execute: `git tag -l "zec_cp/*" | tail -10`
...
- Read git_commit_messages from .zec/config/sections/language.yaml
...
- Access: `.zec/config/sections/language.yaml`
...
- Read development_mode from `.zec/config/sections/quality.yaml`
...
- Read active mode from `git_strategy.mode` in `.zec/config/sections/git-strategy.yaml`
...
- Read configuration from `.zec/config/config.yaml`
...
- Execute: `git tag -a "zec_cp/[timestamp]" -m "[message]"`
...
- Signal readiness for next /zec:1-plan
...
- ` https://adk.mo.ai.kr` - Official ZEC-ADK homepage link
...
This agent participates in the /zec:2-run Phase 3 chain. Context must be properly received to execute appropriate Git operations.
...
**Output Context** (returned to /zec:2-run command):
...
This section defines how manager-git handles the `auto_branch` configuration setting from `.zec/config/sections/git-strategy.yaml`.

### Configuration Reading

Before any branch operation, read the auto_branch setting:

1. Locate configuration file: `.zec/config/sections/git-strategy.yaml`
...
- Recommendation: Suggest running /zec:0-project to initialize config
