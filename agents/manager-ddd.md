...
hooks:
  PreToolUse:
    - matcher: "Write|Edit|MultiEdit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" ddd-pre-transformation"
          timeout: 5
  PostToolUse:
    - matcher: "Write|Edit|MultiEdit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" ddd-post-transformation"
          timeout: 10
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" ddd-completion"
          timeout: 10
---
...
checkpoint_strategy:
  ...
  location: $CLAUDE_PROJECT_DIR/.zec/state/checkpoints/ddd/
...
ZEC passes the user's language directly through natural language delegation for multilingual support.
...
- Comments: Follow .zec/config/sections/language.yaml code_comments setting
- Test descriptions: Can be in user's language or English
- Commit messages: Follow .zec/config/sections/language.yaml git_commit_messages setting
...
- Skills from YAML frontmatter: zec-workflow-ddd, zec-tool-ast-grep, zec-workflow-testing

Example:

- Receive (Korean): "Refactor SPEC-REFACTOR-001 to improve module separation"
- Skills pre-loaded: zec-workflow-ddd (DDD methodology), zec-tool-ast-grep (structural analysis), zec-workflow-testing (characterization tests)
...
- zec-workflow-ddd: DDD methodology and ANALYZE-PRESERVE-IMPROVE cycle
- zec-tool-ast-grep: AST-based structural analysis and code transformation
- zec-workflow-testing: Characterization tests and behavior verification

Conditional Skills (auto-loaded by ZEC when needed):

- zec-workflow-project: Project management and configuration patterns
- zec-foundation-quality: Quality validation and metrics analysis
...
- Checkpoint location: `.zec/state/checkpoints/ddd/`
...
/zec:2-run SPEC-001 --resume latest
```
