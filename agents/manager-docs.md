...
description: |
  ...
  Absorbs: OMC writer (lightweight doc writing) and OMC document-specialist (external reference lookup, conditional capability).
...
hooks:
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" docs-verification"
          timeout: 10
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" docs-completion"
          timeout: 10
---
...
checkpoint_strategy:
  ...
  # CRITICAL: Always use project root for .zec to prevent duplicate .zec in subfolders
  location: $CLAUDE_PROJECT_DIR/.zec/state/checkpoints/docs/
...
- Checkpoint location: `.zec/state/checkpoints/docs/`
...
/zec sync SPEC-AUTH-001

# Resume from checkpoint after crash
/zec sync SPEC-AUTH-001 --resume latest
```
...
- zec-foundation-core: SPEC-first DDD, TRUST 5 framework, execution rules
- zec-workflow-docs: Documentation workflow, validation scripts
- zec-library-mermaid: Mermaid diagram creation and validation
- zec-foundation-claude: Claude Code authoring patterns, skills/agents/commands
- zec-library-nextra: Nextra framework setup and optimization

# Conditional skills (auto-loaded by ZEC when needed)

conditional_skills = [
"zec-domain-uiux", # WCAG compliance, accessibility patterns, Pencil MCP integration
"zec-lang-python", # Python documentation patterns
"zec-lang-typescript", # TypeScript documentation patterns
"zec-workflow-project", # Project documentation management
"zec-ai-nano-banana" # AI content generation
]
...
### 1. ZEC-ADK Ecosystem Integration

**ZEC-ADK Component Integration Workflow:**

Coordinate documentation workflows with existing ZEC-ADK components:
...
- **Documentation Synchronization**: Sync Nextra docs with .zec/docs/ directory structure
...
run: |
npx @zec/nextra-expert generate \\
--source ./src \\
--output ./docs \\
...
run: |
npx @zec/docs-linter validate ./docs
npx @zec/mermaid-validator check ./docs
...
Use ZEC delegation to generate comprehensive documentation:

```bash
# Delegation instruction for ZEC
...
Use ZEC delegation for specialized documentation requirements:
