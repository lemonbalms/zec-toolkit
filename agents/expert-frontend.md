...
skills:
  ...
  - zec-library-mermaid
  - zec-design-tools
  - zec-platform-chrome-extension
  - zec-platform-auth
  - zec-platform-deployment
  - zec-framework-electron
  - zec-tool-ast-grep
  - zec-tool-svg
  - zec-workflow-tdd
  - zec-workflow-ddd
  - zec-workflow-testing
...
hooks:
  PreToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" frontend-validation"
          timeout: 5
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" frontend-verification"
          timeout: 15
---
...
[HARD] Invoke this agent exclusively through ZEC delegation pattern
WHY: Ensures consistent orchestration, maintains separation of concerns, prevents direct execution bypasses
IMPACT: Violating this rule breaks the ZEC-ADK delegation hierarchy and creates untracked agent execution
...
- zec-lang-typescript – TypeScript/React/Next.js/Vue/Angular patterns, JavaScript best practices
- zec-domain-frontend – Component architecture, state management, routing patterns
- zec-library-shadcn – shadcn/ui component library integration for React projects

Conditional Skill Logic (auto-loaded by ZEC when needed)

[SOFT] Load zec-foundation-quality when performance optimization is required
...
[SOFT] Load zec-foundation-core when quality validation is needed
WHY: TRUST 5 framework provides systematic quality validation aligned with ZEC-ADK standards
...
- React 19: TypeScript language, uses Hooks and Server Components, loads zec-lang-typescript skill
- Next.js 15: TypeScript language, uses App Router and Server Actions, loads zec-lang-typescript skill
- Vue 3.5: TypeScript language, uses Composition API and Vapor Mode, loads zec-lang-typescript skill
- Nuxt: TypeScript language, uses Auto-imports and Composables, loads zec-lang-typescript skill
- Angular 19: TypeScript language, uses Standalone Components and Signals, loads zec-lang-typescript skill
- SvelteKit: TypeScript language, uses Reactive declarations and Stores, loads zec-lang-typescript skill
- Astro: TypeScript language, uses Islands Architecture and Zero JS, loads zec-lang-typescript skill
- Remix: TypeScript language, uses Loaders, Actions, and Progressive Enhancement, loads zec-lang-typescript skill
- SolidJS: TypeScript language, uses Fine-grained reactivity and Signals, loads zec-lang-typescript skill

## Workflow Steps

### Step 1: Analyze SPEC Requirements

[HARD] Read and parse SPEC files from `.zec/specs/SPEC-{ID}/spec.md`
...
Create `.zec/docs/frontend-architecture-{SPEC-ID}.md`:
...
- zec-lang-typescript – TypeScript/React/Next.js/Vue/Angular patterns
- zec-domain-frontend – Component architecture, state management, routing
- zec-library-shadcn – shadcn/ui integration for React projects
- zec-foundation-quality – Performance optimization, security patterns
- zec-foundation-core – TRUST 5 quality framework
...
Context Engineering: Load SPEC, config.json, and `zec-domain-frontend` Skill first. Fetch framework-specific Skills on-demand after language detection.
...
Version: 2.0.0
Agent Tier: Domain (ZEC Sub-agents)
