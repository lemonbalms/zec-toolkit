...
- YAML frontmatter configuration with official and ZEC-extended fields
...
- If `--zec` flag is present in the request, use `zec-` prefix instead of `my-`
...
Create SKILL.md and supporting files in `.claude/skills/<prefix>-<name>/` directory (prefix is `my-` by default, or `zec-` with `--zec` flag). Apply frontmatter, write content sections, and verify line count.
...
### ZEC-ADK Extended Fields

These fields are NOT in the official Claude Code spec but are used by ZEC-ADK system skills:
...
[HARD] Default prefix is `my-`. All user-created skills use `my-` prefix unless `--zec` flag is explicitly provided.

- Default: `my-<name>` → directory `.claude/skills/my-<name>/`
- With `--zec` flag: `zec-<name>` → directory `.claude/skills/zec-<name>/`

The `zec-` namespace is reserved for ZEC-ADK system skills. Only use `zec-` prefix when:
- The `--zec` flag is present in the user request
- The user explicitly requests "admin mode", "system skill", or "ZEC-ADK development"
...
[HARD] Skills MUST be created in `.claude/skills/` directory, NEVER in `.zec/skills/`.

[HARD] The skill file MUST always be named `SKILL.md`. NEVER name it after the skill or use any other filename.

[HARD] The full skill name (e.g., `zec-library-pykrx`) is used as a SINGLE directory name. NEVER create nested subdirectories by splitting the name on hyphens. Hyphens are part of the directory name, not path separators.
...
Examples:
- `.claude/skills/zec-library-pykrx/SKILL.md`  ✅ correct
- `.claude/skills/my-analyzing-code/SKILL.md` ✅ correct
- `.claude/skills/zec/library/pykrx.md`          ❌ WRONG: nested dirs + wrong filename
- `.claude/skills/zec-library/pykrx.md`           ❌ WRONG: wrong filename
...
Tool Permissions by ZEC Category:
