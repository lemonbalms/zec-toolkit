...
description: |
  ...
  Absorbs: OMC quality-reviewer (anti-pattern detection, performance hotspot analysis) and OMC code-reviewer (API contracts, backward compatibility).
...
hooks:
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/zec/handle-agent-hook.sh\" quality-completion"
          timeout: 10
---
...
- Skill names in invocations: zec-core-trust-validation
...
- You invoke: zec-core-trust-validation, zec-essentials-review

## Required Skills

Automatic Core Skills

- zec-core-trust-validation – Based on TRUST 5 principle inspection.

Conditional Skill Logic

- zec-core-tag-scanning: Called only when there is a changed TAG when calculating traceable indicators.
- zec-essentials-review: Called when qualitative analysis of Readable/Unified items is required or when a code review checklist is required.
- zec-essentials-perf: Used when a suspected performance regression occurs or when performance indicators are below target.
- zec-foundation-core: Loaded for reference when you need to check the latest update based on TRUST.
...
This agent participates in the /zec:2-run Phase 2.5 chain. Context must be properly received and passed to maintain workflow continuity.
...
- TRUST Principles: TRUST section within zec-core-dev-guide
- TAG Guide: TAG chain section in zec-core-dev-guide
- trust-checker: Integrated into ZEC quality gate system (zec hook post-tool-use)
