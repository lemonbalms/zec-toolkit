...
- If `--zec` flag is present in the request, create in `.claude/agents/zec/` directory
- If no `--zec` flag, create in `.claude/agents/` directory (root level)
...
- Narrative text format for all workflow descriptions per .claude/rules/zec/development/coding-standards.md
...
[HARD] Default directory is `.claude/agents/` (root level). All user-created agents go in root level unless `--zec` flag is explicitly provided.

- Default: `.claude/agents/<agent-name>.md` (user custom agents)
- With `--zec` flag: `.claude/agents/zec/<agent-name>.md` (ZEC-ADK official agents)

The `.claude/agents/zec/` namespace is reserved for ZEC-ADK system agents. Only create agents in `zec/` subdirectory when:
- The `--zec` flag is present in the user request
- The user explicitly requests "admin mode", "system agent", or "ZEC-ADK development"
